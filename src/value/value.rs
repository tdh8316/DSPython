...
impl<'cb, 'ctx: 'cb, T> ValueTypeHandler<'cb, 'ctx, T> {
  ...
  pub fn handle_bool(&mut self, handler: &'cb dyn Fn(&ValueType, IntType<'ctx>) -> T) -> &mut Self {
    self.bool_handler = handler;
    self
  }

  pub fn handle_int(&mut self, handler: &'cb dyn Fn(&ValueType, IntType<'ctx>) -> T) -> &mut Self {
    self.int_handler = handler;
    self
  }
...
impl<'ctx> Value<'ctx> {
  ...
  pub fn from_any_value(value_type: ValueType, any_value: AnyValueEnum<'ctx>) -> Value<'ctx> {
    match any_value {
      ...
      AnyValueEnum::PhiValue(value) => Value::from_basic_value(value_type, value.as_basic_value()),
      _ => panic!("unexpected type encountered"),
    }
  }

  pub fn from_basic_value(value_type: ValueType, basic_value: BasicValueEnum<'ctx>) -> Value<'ctx> {
    match basic_value {
...
impl ValueType {
  ...
  pub fn to_any_type<'ctx>(&self, context: &'ctx Context) -> AnyTypeEnum<'ctx> {
    match self {
      ...
      ValueType::Str => AnyTypeEnum::PointerType(context.i8_type().ptr_type(AddressSpace::Generic)),
    }
  }
  ...
  pub fn invoke_handler<'cb, 'ctx: 'cb, T>(
    ...
    match self {
      ...
      ValueType::U128 => (*value_type_handler.unsigned_int_handler)(self, context.i128_type()),
      ...
      ValueType::Str => {
        (*value_type_handler.str_handler)(self, context.i8_type().ptr_type(AddressSpace::Generic))
      }
    }
  }
